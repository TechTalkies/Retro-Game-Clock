...
// ========== Custom "Arcade Hero" Colors (RGB565) ==========
#define HERO_COLOR 0xFD00  // Golden Orange
#define TIME_PELLET ST77XX_WHITE // Neon Cyan
#define PATH_PELLET 0xF81F // Hot Pink
#define COLON_COLOR 0x07E0 // Bright Green
#define DATE_COLOR 0xFFE0  // Classic Arcade Yellow

// ========== Scaled Grid & Positions ==========
#define PAC_SPACING 6     // Increased from default (wider grid)
#define PAC_PELLET_SIZE 2 // 2x2 pellets (increased from 1)
#define PAC_TIME_Y 44

// Centered X positions for 5 digits (Width: ~132px, Centered on 160px screen)
static const int DIGIT_X_PACMAN[5] = {14, 46, 78, 90, 122};
...
static const PathStep eatingPaths[10][50] = {
    ...
    {{3, 6}, // Bottom stem
     ...
     {0, 4}, // Bar left end
     {1, 4},
     {2, 4}, // Bar continues (3,4 already eaten)
     {4, 4}, // Bar right end
     {255, 255}},
    ...
    {{1, 6}, {2, 6}, {3, 6}, // Bottom arc
     ...
     {4, 4}, // Right side up
     {3, 3},
     {2, 3}, // Middle bar (partial)
     {0, 3},
     {0, 4},
     {0, 5}, // Left side (middle down)
     {0, 2}, // Left upper
     {1, 1}, // Diagonal
     {2, 0},
     {3, 0}, // Top tail
...
void displayClockWithPacman()
{
  ...
  if (!getTimeWithTimeout(&timeinfo))
  {
    ...
    display.setTextColor(DATE_COLOR); // Explicitly color the error/sync text
  ...
  // Date at top (Updated to Arcade Yellow)
  display.setTextSize(2);
  display.setTextColor(DATE_COLOR); // Updated from DISPLAY_WHITE
  char dateStr[12];
  ...
  for (int i = 0; i < 5; i++)
  {
    if (i == 2)
    {
      if (shouldShowColon())
      {
        int colon_x = (DIGIT_X_PACMAN[1] + 4 * PAC_SPACING + DIGIT_X_PACMAN[3]) / 2;
        display.fillCircle(colon_x, PAC_TIME_Y + 12, PAC_PELLET_SIZE, COLON_COLOR);
        display.fillCircle(colon_x, PAC_TIME_Y + 26, PAC_PELLET_SIZE, COLON_COLOR);
      }
      continue;
    }

    int base_y = PAC_TIME_Y + (int)digit_offset_y[i];
    ...
    for (uint8_t row = 0; row < DIGIT_GRID_H; row++)
    {
      for (uint8_t col = 0; col < DIGIT_GRID_W; col++)
      {
        ...
        bool is_eaten = mask_active && (digitEatenPellets[i][byte_idx] & bit_mask) != 0;

        if (!is_eaten)
        {
          int px = base_x + col * PAC_SPACING;
          int py = base_y + row * PAC_SPACING;
          display.fillCircle(px, py, PAC_PELLET_SIZE, TIME_PELLET);
        }
      }
    }
  }
...
void updatePacmanAnimation(struct tm *timeinfo)
{
  ...
  if (seconds >= 56 && !pacman_animation_triggered && pacman_state == PACMAN_PATROL)
  {
    ...
    if (num_targets > 0)
    {
      ...
      if (target_queue_length > 0)
      {
        ...
        float first_x = DIGIT_X_PACMAN[first_idx] + first_step.col * PAC_SPACING;
  ...
  case PACMAN_TARGETING:
    // 2-phase L-shaped movement: horizontal first (along patrol), then vertical to first pellet
    {
      ...
      float target_x = DIGIT_X_PACMAN[target_idx] + first_step.col * PAC_SPACING;
      float target_y = PAC_TIME_Y + first_step.row * PAC_SPACING;
    ...
    pacman_direction = 2; // Face down while returning
    {
      ...
      if (abs(dy) <= speed * 1.5)
      {
        ...
        if (target_queue_index < target_queue_length)
        {
          ...
          float next_x = DIGIT_X_PACMAN[next_idx] + next_step.col * PAC_SPACING;
...
void updatePacmanEating()
{
  ...
  float target_x = digit_base_x + current_step.col * PAC_SPACING;
  float target_y = digit_base_y + current_step.row * PAC_SPACING;
  ...
  for (uint8_t row = 0; row < DIGIT_GRID_H; row++)
  {
    for (uint8_t col = 0; col < DIGIT_GRID_W; col++)
    {
      ...
      float pellet_x = digit_base_x + col * PAC_SPACING;
      float pellet_y = digit_base_y + row * PAC_SPACING;
...
void startEatingDigit(uint8_t digit_index, uint8_t digit_value)
{
  ...
  pacman_x = digit_base_x + first_step.col * PAC_SPACING;
  pacman_y = digit_base_y + first_step.row * PAC_SPACING;
  ...
  {
    float dx = (float)(second_step.col - first_step.col) * PAC_SPACING;
    float dy = (float)(second_step.row - first_step.row) * PAC_SPACING;
    float abs_dx = abs(dx);
    float abs_dy = abs(dy);
    float ratio = (abs_dx > abs_dy) ? (abs_dy / abs_dx) : (abs_dx / abs_dy);
    ...
    {
      if (dx > 0 && dy > 0)
        pacman_direction = 3; // Down-right
      else if (dx < 0 && dy > 0)
        pacman_direction = 4; // Down-left
      else if (dx < 0 && dy < 0)
        pacman_direction = -3; // Up-left
      else
        pacman_direction = -4; // Up-right
    }
    else if (abs_dx > abs_dy)
    {
...
void drawPellets()
{
  for (int i = 0; i < num_pellets; i++)
  {
    if (patrol_pellets[i].active)
    {
      // Hot Pink patrol pellets
      display.fillCircle(patrol_pellets[i].x, PACMAN_PATROL_Y, PAC_PELLET_SIZE, PATH_PELLET);
    }
  }
}
...
void drawPacman(int x, int y, int direction, int mouthFrame)
{
  if (x < -15 || x > SCREEN_WIDTH + 15)
    return;
  if (y < -15 || y > SCREEN_HEIGHT + 15)
    return;

  // Scaled up to 8 radius (16x16 bounding box)
  display.fillCircle(x, y, 8, HERO_COLOR);

  if (mouthFrame > 0)
  {
    // Scaled mouth animation
    int m = (mouthFrame + 1) * 3;

    if (direction == 1)
    { // Right
      display.fillTriangle(x + 1, y, x + 10, y - m, x + 10, y + m, DISPLAY_BLACK);
    }
    else if (direction == -1)
    { // Left
      display.fillTriangle(x - 1, y, x - 10, y - m, x - 10, y + m, DISPLAY_BLACK);
    ...
    { // Down
      display.fillTriangle(x, y + 1, x - m, y + 10, x + m, y + 10, DISPLAY_BLACK);
    }
    else if (direction == -2)
    { // Up
      display.fillTriangle(x, y - 1, x - m, y - 10, x + m, y - 10, DISPLAY_BLACK);
    ...
    { // Down-right
      display.fillTriangle(x, y, x + 9, y + 3, x + 3, y + 9, DISPLAY_BLACK);
    }
    else if (direction == -3)
    { // Up-left
      display.fillTriangle(x, y, x - 9, y - 3, x - 3, y - 9, DISPLAY_BLACK);
  ...
  display.fillRect(ex, ey, 2, 2, DISPLAY_BLACK);
}