...
void displayStandardClock()
{
  ...
  // Native 160x128 ST7735 layout: large centred time, date and day sections.
  display.drawRoundRect(4, 12, SCREEN_WIDTH - 8, 62, 6, DISPLAY_ACCENT);
  ...
  // Center time
  int time_width = 5 * 30; // 5 chars * 30px at text size 5
  int time_x = (SCREEN_WIDTH - time_width) / 2;
  display.setCursor(time_x + 3, 26);
  display.print(timeStr);

  // AM/PM indicator for 12-hour format
  display.setTextColor(DISPLAY_ACCENT);
  drawMeridiemIndicator(132, 16, isPM);
  ...
  int date_x = (SCREEN_WIDTH - 120) / 2;
  display.setCursor(date_x, 84);
  display.print(dateStr);

  // Day of week
  const char *days[] = {"Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"};
  const char *dayName = days[timeinfo.tm_wday];
  int day_width = strlen(dayName) * 12;
  int day_x = (SCREEN_WIDTH - day_width) / 2;
  display.setCursor(day_x, 108);
...
void displayLargeClock()
{
  ...
  // Box now uses nearly the full screen height (was capped at 78px tall,
  // leaving ~40px unused below it) so the time can be as large as possible.
  display.drawRoundRect(3, 4, SCREEN_WIDTH - 6, 120, 8, DISPLAY_ACCENT);

  // Left column: HH stacked above MM, size 6 - the largest that still
  // fits two rows inside the box with margin top/bottom/between.
  char hourStr[3];
  sprintf(hourStr, "%02d", displayHour);
  char minStr[3];
  sprintf(minStr, "%02d", displayMin);
  ...
  // AM/PM indicator, top-right corner.
  display.setTextColor(DISPLAY_ACCENT);
  drawMeridiemIndicator(130, 14, isPM);

  // Right column: day-of-month number on top, weekday abbreviation below.
  static const char *WEEKDAY_ABBREV[7] = {"SUN", "MON", "TUE", "WED", "THU", "FRI", "SAT"};

  char dayNumStr[3];
  sprintf(dayNumStr, "%02d", timeinfo.tm_mday);
  ...
  display.print(WEEKDAY_ABBREV[timeinfo.tm_wday]);
